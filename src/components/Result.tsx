...
export const Result = () => {
  ...
  return (
    <div className='lg:col-span-7 h-full'>
      <div
        className='bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-sky-100/50 h-full min-h-100 flex flex-col relative overflow-hidden border border-sky-50'>
        <div className='flex justify-between items-center mb-6'>
          <h3 className='font-bold text-lg text-gray-900'>Ваше поздравление</h3>
          {generatedText && !loading && (
            <button
              onClick={handleCopy}
              className='text-gray-400 hover:text-sky-600 transition-colors p-2 rounded-full hover:bg-sky-50'
              title='Копировать'
            >
